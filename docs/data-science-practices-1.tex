...
The configuration file is simply a plain-text yaml file read by the \href{https://CRAN.R-project.org/package=config}{config} package. It is great when a value has to be coordinated across multiple file
...
\hypertarget{data-rest}{%
\chapter{Data at Rest}\label{data-rest}}
...
\hypertarget{data-rest-csv}{%
\subsection{csv}\label{data-rest-csv}}
...
Incoming files should be plain-text \href{https://en.wikipedia.org/wiki/Comma-separated_values}{csv} --not Excel or other proprietary or binary format.

\hypertarget{data-rest-rds}{%
\subsection{rds}\label{data-rest-rds}}

\hypertarget{data-rest-sqlite}{%
\subsection{SQLite}\label{data-rest-sqlite}}

\hypertarget{data-rest-database}{%
\subsection{Central Enterprise database}\label{data-rest-database}}

\hypertarget{data-rest-redcap}{%
\subsection{Central REDCap database}\label{data-rest-redcap}}

\hypertarget{data-rest-avoid}{%
\subsection{Containers to avoid for raw/input}\label{data-rest-avoid}}
...
\hypertarget{data-rest-conventions}{%
\section{Storage Conventions}\label{data-rest-conventions}}

\hypertarget{data-rest-conventions-all}{%
\subsection{All Sources}\label{data-rest-conventions-all}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{date format}: use \texttt{YYYY-MM-DD} (\href{https://www.explainxkcd.com/wiki/index.php/1179:_ISO_8601}{ISO-8601})
\item
  \textbf{time format}: use \texttt{HH:MM} or \texttt{HH:MM:SS}. Use a leading zero from midnight to 9:59am, with a colon separating hours, minutes, and seconds (\emph{i.e.}, \textbf{0}9:59am)
\item
  \textbf{patient names}: separate the \texttt{name\_last}, \texttt{name\_first}, and \texttt{name\_middle} when possible.
\item
  \textbf{consistency across versions}: Use a script to produce the data sent to us, and inform us when changes occur. Most of our processes are automated, and changes that are trivial to humans (\emph{e.g.}, \texttt{yyyy-mm-dd} to \texttt{mm/dd-yy}) cause problems for the automation.
\item
...
\hypertarget{data-rest-conventions-text}{%
\subsection{Text}\label{data-rest-conventions-text}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{csv}: comma separated values are the most common plain-text format, so they have better support than similar formats where cells are separated by tabs or semi-colons. However, if you are receiving a well-behaved file separated by these characters, just go with the flow.
\item
  \textbf{cells enclosed in quotes}: a `cell' should be enclosed in double quotes, especially if it's a string/character variable.
\end{enumerate}

\hypertarget{data-rest-conventions-excel}{%
\subsection{Excel Exceptions}\label{data-rest-conventions-excel}}
...
\tightlist
\item
  \textbf{avoid multiple tabs/worksheets}: they are more complicated to read with automation, and the produces the opportunities for inconsistent variables across tabs/worksheets.
\end{enumerate}

\hypertarget{data-rest-conventions-meditech}{%
\subsection{Meditech}\label{data-rest-conventions-meditech}}
...
\hypertarget{data-rest-conventions-database}{%
\subsection{Databases}\label{data-rest-conventions-database}}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Don't use Excel for data storage
  ...
  As much as Excel interferes with our EMR work, we're lucky. It has messed with other branches of science much worse. Genomics were using it far too late before they realized their mistakes. I'm guessing some of them still haven't learned. \url{https://qz.com/768334/years-of-genomics-research-is-riddled-with-errors-thanks-to-a-bunch-of-botched-excel-spreadsheets/}
