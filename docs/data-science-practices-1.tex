...
\item
  The ``pipe'' function (declared in the \texttt{magrittr\textquotesingle{}\ package\ ,\ *i.e.*,}\%\textgreater\%\texttt{)\ is\ attached\ with}import::from(magrittr, ``\%\textgreater\%'')`. This frequently-used function is available without qualification.
\item
  Our analysis files tend to use many functions in a few concentrated packages (compared to manipulation files), so conflicting function names are less common. Typical packages used in analysis are `ggplot2' and `lme4'.
\end{enumerate}
...
\url{http://r-pkgs.had.co.nz/namespace.html\#search-path} about \texttt{library()}, \texttt{requireNamespace()}, and their siblings, as well as the larger concepts such as attaching functions into the search path.
...
\KeywordTok{requireNamespace}\NormalTok{(}\StringTok{"dplyr"}\NormalTok{     ) }\CommentTok{# Don't attach. Its function names conflict many packages (esp base, stats, and plyr)}
\KeywordTok{requireNamespace}\NormalTok{(}\StringTok{"rlang"}\NormalTok{     ) }\CommentTok{# Language constucts, like quosures}
\KeywordTok{requireNamespace}\NormalTok{(}\StringTok{"checkmate"}\NormalTok{ ) }\CommentTok{# Asserts expected conditions requireNamespace("OuhscMunge") # remotes::install_github(repo="OuhscBbmc/OuhscMunge")}
\end{Highlighting}
\end{Shaded}
