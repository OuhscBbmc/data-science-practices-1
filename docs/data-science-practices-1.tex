...
\ControlFlowTok{if}\NormalTok{( }\OperatorTok{!}\NormalTok{base}\OperatorTok{::}\KeywordTok{requireNamespace}\NormalTok{(}\StringTok{"OuhscMunge"}\NormalTok{) ) remotes}\OperatorTok{::}\KeywordTok{install\_github}\NormalTok{(}\StringTok{"OuhscBbmc/OuhscMunge"}\NormalTok{)}
...
\subsection{GitHub Desktop}\label{workstation-github-client}}

\href{http://desktop.github.com/}{GitHub Desktop} does the basic tasks a little easier than the git features built into RStudio. This client is available for Windows and macOS. (Occasionally, someone might need to use git from the command line to fix problems, but this is not required to start.)

\hypertarget{workstation-rtools}{%
\subsection{R Tools}\label{workstation-rtools}}

\href{https://cran.r-project.org/bin/windows/Rtools/}{R Tools for Windows} is necessary to build some packages in development hosted on GitHub. If running Linux, the components of R Tools are likely already installed on your machine. \{added Feb 2017\}
...
\end{Shaded}
\item
  \textbf{\href{https://www.python.org/downloads/}{Standard Python}}, while installing packages through pip3 in the terminal. If the \texttt{pip3} command is unrecognized because it's missing from the OS \href{https://en.wikipedia.org/wiki/PATH_(variable)}{path} variable, an alternative is \texttt{py\ -3\ -mpip\ install\ pysftp}; this calls pip through the \texttt{py} command which is usually in the path variable after installation.
\end{itemize}
...
\hypertarget{workstation-winscp}{%
\subsection{WinSCP (SSMS)}\label{workstation-winscp}}

\href{https://winscp.net/eng/download.php}{WinSCP} is a GUI for SCP and SFTP file transfer using SSH keys. The tool is occassionally useful for admins when collaborating with other institutions or \href{https://www.ou.edu/oscer}{other OU computing resources}. Because PHI can accidentally be sent to collaborators without a DUA, we recommend that WinSCP be installed only informed administrators. The typical data scientist on our teams does not need this tool.
...
\hypertarget{git-contribution}{%
\section{Steps for Contributing to Repo}\label{git-contribution}}

\hypertarget{git-contribution-regular}{%
\subsection{Regular Contributions}\label{git-contribution-regular}}

\hypertarget{git-contribution-regular-pull}{%
\subsubsection{Keep your dev branch fresh}\label{git-contribution-regular-pull}}

We recommend doing this at least every day you write code in a repo. Perhaps more frequently if a lot of developers are pushing code (\emph{e.g.}, right before a reporting deadline).

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Update master on your local machine (from the GitHub server)
\item
  Merge master \emph{into} your local dev branch
\item
  Push your local dev branch to the GitHub server
\end{enumerate}

\hypertarget{git-contribution-regular-push}{%
\subsubsection{Make your code contributions available to other analysts}\label{git-contribution-regular-push}}

At least every few days, push your changes to the master branch so teammates can benefit from your work. Especially if you are improving the pipeline code (\emph{e.g.} Ellises or REDCap Arches)

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Make sure you dev branch is updated immediately before you create a \href{}{Pull Request}. Follow the \protect\hyperlink{git-contribution-regular-pull}{steps above}.
\item
  Verify the merged code still works as expected. In other words, make sure that when your new code is blended with the newest master code, nothing breaks. Depending on the repo, these steps might include

  \begin{enumerate}
  \def\labelenumii{\arabic{enumii}.}
  \tightlist
  \item
    \href{https://support.rstudio.com/hc/en-us/articles/200486508-Building-Testing-and-Distributing-Packages}{Build and Check} the repo (assuming the rep is also a package).
  \item
    Run any code that verify's the basic functionality of the repo. (For example, our MIECHV team should run ``high-school-funnel.R'' and verify the assertions passed).
  \end{enumerate}
\item
  Commit changes in your dev branch and push to the GitHub server.
\item
  Create a \href{https://docs.github.com/en/free-pro-team@latest/github/collaborating-with-issues-and-pull-requests/creating-a-pull-request}{Pull Request} (otherwise known as a PR) and \href{https://docs.github.com/en/free-pro-team@latest/github/collaborating-with-issues-and-pull-requests/about-pull-request-reviews}{assign a reviewer}. (For example, developers in the MIECHV team are paired together to review each other's code.)
\item
  The reviewer will pull your dev branch on to their local machine and run the same checks and verification (that you did on the 2nd step above). This duplicate effort helps verify that your code likely works for everyone on their own machines.
\item
  The reviewer then accepts the PR and the master branch now contains your changes and are available to teammates.
\end{enumerate}
...
\NormalTok{col\_types \textless{}{-}}\StringTok{ }\KeywordTok{list}\NormalTok{(}
  \DataTypeTok{sak                      =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_integer}\NormalTok{(),  }\CommentTok{\# "system{-}assigned key"}
  \DataTypeTok{aid\_category\_id          =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_character}\NormalTok{(),}
  \DataTypeTok{age                      =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_integer}\NormalTok{(),}
  \DataTypeTok{service\_date\_first       =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_date}\NormalTok{(}\StringTok{"\%m/\%d/\%Y"}\NormalTok{),}
  \DataTypeTok{service\_date\_lasst       =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_date}\NormalTok{(}\StringTok{"\%m/\%d/\%Y"}\NormalTok{),}
  \DataTypeTok{claim\_type               =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_character}\NormalTok{(),}
  \DataTypeTok{provider\_id              =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_character}\NormalTok{(),}
  \DataTypeTok{provider\_lat             =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_double}\NormalTok{(),}
  \DataTypeTok{provider\_long            =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_double}\NormalTok{(),}
  \DataTypeTok{provider\_zip             =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_character}\NormalTok{(),}
  \DataTypeTok{cpt                      =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_integer}\NormalTok{(),}
  \DataTypeTok{revenue\_code             =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_integer}\NormalTok{(),}
  \DataTypeTok{icd\_code                 =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_character}\NormalTok{(),}
  \DataTypeTok{icd\_sequence             =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_integer}\NormalTok{(),}
  \DataTypeTok{vocabulary\_coarse\_id     =}\NormalTok{ vroom}\OperatorTok{::}\KeywordTok{col\_integer}\NormalTok{()}
\NormalTok{)}

\CommentTok{\# {-}{-}{-}{-} load{-}data {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{ds \textless{}{-}}\StringTok{ }\NormalTok{vroom}\OperatorTok{::}\KeywordTok{vroom}\NormalTok{(}
  \DataTypeTok{file      =}\NormalTok{ config}\OperatorTok{$}\NormalTok{path\_ohca\_patient,}
  \DataTypeTok{delim     =} \StringTok{"}\CharTok{\textbackslash{}t}\StringTok{"}\NormalTok{,}
  \DataTypeTok{col\_names =} \KeywordTok{names}\NormalTok{(col\_types),}
  \DataTypeTok{col\_types =}\NormalTok{ col\_types}
\NormalTok{)}
