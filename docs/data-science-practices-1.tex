...
\FunctionTok{path_subject_1_raw}\KeywordTok{:}\AttributeTok{  }\StringTok{"data-public/raw/subject-1.csv"}
\FunctionTok{path_mlm_1_raw}\KeywordTok{:}\AttributeTok{      }\StringTok{"data-public/raw/mlm-1.csv"}

\CommentTok{# Central Database (produced by Ellis lanes).}
\FunctionTok{path_database}\KeywordTok{:}\AttributeTok{       }\StringTok{"data-public/derived/db.sqlite3"}

\CommentTok{# Analysis-ready datasets (produced by scribes & consumed by analyses).}
\FunctionTok{path_mlm_1_derived}\KeywordTok{:}\AttributeTok{  }\StringTok{"data-public/derived/mlm-1.rds"}

\CommentTok{# Metadata}
\FunctionTok{path_annotation}\KeywordTok{:}\AttributeTok{     }\StringTok{"data-public/metadata/cqi-annotation.csv"}

\CommentTok{# Logging errors and messages from automated execution.}
\FunctionTok{path_log_flow}\KeywordTok{:}\AttributeTok{       !expr strftime(Sys.time(), "data-unshared/log/flow-%Y-%m-%d--%H-%M-%S.log")}

\CommentTok{# time_zone_local       :  "America/Chicago" # Force local time, in case remotely run.}

\CommentTok{# ---- Validation Ranges & Patterns ----}
\FunctionTok{range_record_id         }\KeywordTok{:}\AttributeTok{ !expr c(1L, 999999L)}
\FunctionTok{range_dob               }\KeywordTok{:}\AttributeTok{ !expr c(as.Date("2010-01-01"), Sys.Date() + lubridate::days(1))}
\FunctionTok{range_datetime_entry    }\KeywordTok{:}\AttributeTok{ !expr c(as.POSIXct("2019-01-01", tz="America/Chicago"), Sys.time())}
\FunctionTok{max_age                 }\KeywordTok{:}\AttributeTok{ }\DecValTok{25}
\FunctionTok{pattern_mrn_centricity  }\KeywordTok{:}\AttributeTok{ }\StringTok{"^}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{d\{16\}$"}\CommentTok{        # The 64-bit int is more easily validated as a character.}
\end{Highlighting}
\end{Shaded}
\item
  \texttt{flow.R}
